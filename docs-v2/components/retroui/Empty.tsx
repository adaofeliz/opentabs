...
const EmptyTitle = ({ className, ...props }: IEmptyProps) => {
  return <Text as="h3" className={cn('text-lg font-bold md:text-2xl', className)} {...props} />;
};
EmptyTitle.displayName = 'Empty.Title';

const EmptySeparator = ({ className, ...props }: IEmptyProps) => {
  return <div role="separator" className={cn('bg-primary h-1 w-full', className)} {...props} />;
};
