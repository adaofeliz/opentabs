...
All fetch utilities (`fetchJSON`, `postJSON`, `putJSON`, `patchJSON`, `deleteJSON`) accept an optional Zod schema as the last parameter. When provided, the response is validated at runtime and the return type is inferred from the schema — no separate type annotation needed:
...
// Works the same for POST, PUT, PATCH, DELETE
