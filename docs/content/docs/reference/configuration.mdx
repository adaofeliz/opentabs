...
Resolution order: `skipPermissions` (CLI flag / env var) → per-tool override → plugin default → `'off'`.
...
The `--dangerously-skip-permissions` flag on `opentabs start` or the `OPENTABS_SKIP_PERMISSIONS=1` environment variable bypasses all permission checks — all tools become `'auto'`. **This is dangerous and intended only for CI/testing environments.** In normal usage, configure per-plugin and per-tool permissions via the side panel or `config.json`.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>_(unset)_</Table.Cell>
      <Table.Cell>Set to `1` to bypass all permission checks — all tools become `'auto'`. Equivalent to `--dangerously-skip-permissions`. For CI/testing only.</Table.Cell>
    </Table.Row>
    <Table.Row>
